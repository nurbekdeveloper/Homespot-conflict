...
import MainContact from '../contact/Contact';
...
const PersonalProject = () => {
  ...
  const filterProject = houses.filter(c => c.type === 'project');
    ...
    useEffect(() => {
      const loadImage = async () => {
        ...
        img.onload = () => {
          ...
          watermark([img])
            .image(watermark.text.lowerRight(watermarkText, '48px Arial', '#FFF', 0.5))
    ...
    <>
      <div className="house-grid-container">
        <div className="house-grid">
          {filterProject.map(house => (
            <div key={house.id} className="house-card" onClick={() => navigate(`/moreinformation/${house.url}`)}>
              <WatermarkedImage src={house.image} watermarkText="swedstandard.ru" />
